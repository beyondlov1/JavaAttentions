...
function getBoardForServer(result) {
    var boardForServer = [];
    for (var i = 0; i < result.length; i++) {
        ...
        boardForServer.push(line);
    }
    return boardForServer;
}

function sendLoseMessageToServer(losePlayer) {
    $.ajax({
        type: "POST",
        url: "/result?losePlayer="+losePlayer,
        data: JSON.stringify(getBoardForServer(result)),
        dataType: "json",
        ...
        if(writeResult(pos,nextTurnType)){
            ...
            if(judge(result)){
                if(nextTurnType == $("#myType").val()){
                    alert("恭喜你赢了");
                    sendLoseMessageToServer($("#myType").val()==4?"PLAYER1":"PLAYER2");
                }else{
                    alert("你输了");
                }
                clear(canvas);
                ...
                if (isDrew(result)) {
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            var pos = data;
                            var nextTurnType = pos.player == "PLAYER1"?1:4;
                            if(writeResult(pos,nextTurnType)){
                                draw(canvas,pos,nextTurnType);
                                if(judge(result)){
                                    if(nextTurnType == $("#myType").val()){
...
function writeResult(pos,type){
    if(result[pos.x][pos.y] == 0){
        result[pos.x][pos.y]= type;
        return true;
    }else{
        return false;
    }
