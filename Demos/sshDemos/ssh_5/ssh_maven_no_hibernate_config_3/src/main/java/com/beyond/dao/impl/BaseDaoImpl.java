...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	@SuppressWarnings("unchecked")
	public List<T> selectAllByPage(Class<T> c, Page<T> page) {
		DetachedCriteria criteria = DetachedCriteria.forClass(c);
		List<T> findByPage = (List<T>) hibernateTemplate.findByCriteria(criteria, page.getStartRecordIndex(),
	...
	public Long count(Class<T> c) {
		...
		Long count = hibernateTemplate.executeWithNativeSession(new HibernateCallback<Long>() {
			...
			public Long doInHibernate(Session session) throws HibernateException {
				return (Long) session.createQuery("select count(*) from " + c.getName()).uniqueResult();
			}
		});
		return count;
	}
